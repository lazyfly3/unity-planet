    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f input) : SV_Target
            {
                ...
                float2 uv = float2(
                    atan2(direction.z, direction.x) / 6.2831853 + 0.5,
                    asin(direction.y) / 3.1415926 + 0.5);
                float seedOffset = frac(_CloudSeed * 0.000173);
                float windPhase = _Time.y * _ScrollSpeed * max(0.25, _Wind.w);
                float noiseA = tex2D(
                    _MainTex,
                    uv * 2.1 + float2(seedOffset + windPhase, seedOffset * 0.37 + windPhase * 0.31)).r;
                float noiseB = tex2D(
                    _MainTex,
                    uv.yx * 4.2 + float2(seedOffset * 0.61 - windPhase * 0.37,
                        seedOffset + windPhase * 0.19)).r;
                float density = noiseA * 0.68 + noiseB * 0.32;
                density = floor(density * 6.0) / 6.0;
                float threshold = 1.02 - _Coverage;
                float alpha = smoothstep(
                    threshold - _Softness,
                    threshold + _Softness,
                    density) * _Opacity * saturate(_Coverage * 1.45);
