...
namespace ModularAssembly
{
    ...
    public static class ModularPresetPreviewRenderer
    {
        struct PreviewPoint
        {
            public float x;
            ...
            int height = 144)
        {
            var texture = new Texture2D(
                ...
                true)
            {
                name = "ModularPresetPreview",
                hideFlags = HideFlags.HideAndDontSave,
                filterMode = FilterMode.Bilinear,
                wrapMode = TextureWrapMode.Clamp
            };
            Color background = new Color(0.025f, 0.07f, 0.09f, 1f);
            ...
            float scale = Mathf.Min(
                (width - 24f) / Mathf.Max(1f, maxX - minX + 1f),
                (height - 20f) / Mathf.Max(1f, maxY - minY + 1f));
            int radius = Mathf.Clamp(
                Mathf.RoundToInt(scale * 0.34f),
                2,
                7);
            foreach (PreviewPoint point in points.OrderBy(item => item.y))
            {
                int px = Mathf.RoundToInt(
                    12f + (point.x - minX + 0.5f) * scale);
                int py = Mathf.RoundToInt(
                    10f + (point.y - minY + 0.5f) * scale);
                DrawDiamond(
                    pixels,
                    width,
                    height,
        ...
        static Color CategoryColor(GridModuleCategory category)
        {
            switch (category)
            {
                ...
                case GridModuleCategory.RcsThruster:
                    return new Color(0.13f, 0.82f, 1f, 1f);
                case GridModuleCategory.KineticWeapon:
                    return new Color(1f, 0.3f, 0.18f, 1f);
                case GridModuleCategory.Battery:
                    return new Color(0.32f, 1f, 0.55f, 1f);
                default:
                    return new Color(0.72f, 0.88f, 0.94f, 1f);
            }
        }
    }
}
